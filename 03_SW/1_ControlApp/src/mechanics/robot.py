...
class Robot(RobotCalculationMixin):
    ...
    def set_motor_angles(
        self, angle_1: float | int | None, angle_2: float | int | None
    ) -> None:
        """
    ...
    def get_points(self) -> tuple:
        point_0: tuple = (0, 0)
        point_1: tuple = (
            self.settings.LENGTHS[0] * math.cos(self._internal_angles[0]),
            self.settings.LENGTHS[0] * math.sin(self._internal_angles[0]),
        )
        point_2: tuple = (
            point_1[0] + self.settings.LENGTHS[1] * math.cos(self._internal_angles[1]),
            point_1[1] + self.settings.LENGTHS[1] * math.sin(self._internal_angles[1]),
        )
        point_3: tuple = (self.settings.LENGTHS[4], 0)
        point_4: tuple = (
            point_3[0] + self.settings.LENGTHS[2] * math.cos(self._internal_angles[2]),
            point_3[1] + self.settings.LENGTHS[2] * math.sin(self._internal_angles[2]),
        )
        point_5: tuple = (
            point_4[0] + self.settings.LENGTHS[3] * math.cos(self._internal_angles[3]),
    ...
    def get_limits(self) -> list:
        ...
        for i in range(628):
            angle: float = i / 100
            mid_point_circle_1: tuple = (
                self.settings.LENGTHS[0] * math.cos(angle),
                self.settings.LENGTHS[0] * math.sin(angle),
            )
            circle_1: Circle = Circle(
                mid_point_circle_1, self.settings.LENGTHS[1] + self.settings.LENGTHS[3]
            )
            mid_point_circle_2: tuple = (self.settings.LENGTHS[4], 0)
            ...
            try:
                ...
                points.append(
                    (
                        (mid_point_circle_1[0] + point_of_interest[0]) / 2,
                        (mid_point_circle_1[1] + point_of_interest[1]) / 2,
                    )
                )
    ...
    def _recalculate(self):
        ...
        alfa = self.law_of_cosine_angle(
            self.settings.LENGTHS[1], self.settings.LENGTHS[0], rm_abs
        )
        beta = self.law_of_cosine_angle(
            self.settings.LENGTHS[0], self.settings.LENGTHS[1], rm_abs
        )
        ...
        beta = self.law_of_cosine_angle(
            self.settings.LENGTHS[2], self.settings.LENGTHS[3], rm_abs
        )
        ...
        self._internal_angles = [
            angle if abs(angle) > 0.05 else 0 for angle in self._internal_angles
        ]
