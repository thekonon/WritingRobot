...
class RobotCalculationMixin:
    ...
    @staticmethod
    def check_if_point_is_in_both_circles(
        point: List[float] | Tuple[float, float], r1: float, r2: float, l5: float
    ) -> bool:
    ...
    def find_circle_intersections(
        ...
        if d > r1 + r2 or d < abs(r1 - r2) or (d == 0 and r1 == r2):
            raise ValueError(
                "No intersection point found - no solution"
            )  # No intersections or circles are coincident
...
class RobotPropertiesMixin:
    ...
    _lengths: Tuple[float, float, float, float, float]
    ...
    @property
    def lengths(self) -> Tuple[float, float, float, float, float]:
        return self._lengths

    @lengths.setter
    def lengths(self, value: Tuple[float, float, float, float, float]):
        if len(value) == 5:
            self._lengths = value
        else:
