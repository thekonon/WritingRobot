...
from PySide6.QtCore import Qt, QPointF, QRectF, QSize # type: ignore
...
class Drawer:
    ...
    def draw(self) -> None:
        self.scene.clear()
        lengths: tuple[float] = self.robot.lengths
        ...
        end_point = QPointF(x3, y3)
        self.draw_line(start_point, end_point)

        # Line 4
        x4: float = x2 + lengths[3] * cos(phis[3])
        y4: float = y2 + lengths[3] * sin(phis[3])
        start_point = QPointF(x2, y2)
        end_point = QPointF(x4, y4)
        self.draw_line(start_point, end_point)
    ...
    def draw_line(self, start_point: QPointF, end_point: QPointF) -> None:
        ...
        pen: QPen = QPen(QColor(0, 0, 0))  # Set color to black
        pen.setWidth(3)
        line.setPen(pen)
    ...
    def draw_limit_circles(self) -> None:
        ...
        circle1: QGraphicsEllipseItem = QGraphicsEllipseItem(
            rect_1
        )  # Circle centered at (0, 0) with radius 100
        ...
        circle2: QGraphicsEllipseItem = QGraphicsEllipseItem(
            rect_2
        )  # Circle centered at (0, 50) with radius 100
